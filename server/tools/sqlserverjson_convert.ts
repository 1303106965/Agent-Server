 ...
 * 保存文件
 */
function saveFile(dir: string, filename: string, content: string) {
  // 自动创建目录
...
function whereNodeToSql(node: WhereNode | WhereLeaf): string {
  ...
  switch (type) {
    case "EQ":
      return `${key} = ${typeof value === "string" ? sqlEscape(value) : value}`;
    case "NE":
      return `${key} != ${
        typeof value === "string" ? sqlEscape(value) : value
      }`;
    case "GT":
      return `${key} > ${typeof value === "string" ? sqlEscape(value) : value}`;
    case "GE":
    ...
    case "LIKE":
      return `${key} LIKE ${sqlEscape(value)}`;
    case "IN":
      return `${key} IN (${
        Array.isArray(value)
          ? value
              .map((v) => (typeof v === "string" ? sqlEscape(v) : v))
              .join(", ")
          : ""
      })`;
    case "IS_NULL":
...
function jsonToSql(config: ConfigRoot): string {
  ...
  // 构建 SELECT 子句
  const selectParts = data.columns
    .map((col) => `${col.expression} AS ${col.alias}`)
    .join(", ");
...
program
  .name("sqlserverjson_convert")
  .description("在 sqlserverjson 配置（JSON）与 T-SQL 之间相互转换");
...
program
  ...
  .action((opts) => {
    try {
      ...
      const sqlPath = saveFile("SqlServerJson/selSql", `${now}-query.sql`, sql);
